...
wd = paste0(pd, 'figures/scripts/')
outd = paste0(pd, 'figures/outputs/cur_gif/')
...
figout = paste0(outd,'cur_gif')



for (m in 1:length(colz)){

	png(paste0(figout,m,'.png'), width = 12, height = 6, units = 'in', res = 600)
...
#	title(main = paste(m), col.main = pal[m], cex.main = 4)
	...
	mtext('Discharge (L/s)', side = 2, line = 3, cex = 0.5)
